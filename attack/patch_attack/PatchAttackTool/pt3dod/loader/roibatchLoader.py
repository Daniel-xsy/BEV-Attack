...
class roibatchLoader(data.Dataset):
  def __init__(self, roidb, ratio_list, ratio_index, batch_size, imdb, training=True, normalize=None):
    self._roidb = roidb
    self.imdb = imdb
  ...
  def __getitem__(self, index):
    ...
    if True: #self.training:
        ...
        billboards_xyzrpy = [np.array(self.imdb.billboards_camera_positions[index]['billboard_xyzrpy']), 
                            np.array(self.imdb.billboards_camera_positions[index]['billboard2_xyzrpy'])]
        extrinsic_left, intrinsics_left = self.imdb.build_extrinsics(np.array(self.imdb.billboards_camera_positions[index]['camera_xyzrpy']), 
                                          billboards_xyzrpy)
        extrinsic_right, intrinsics_right = self.imdb.build_extrinsics(np.array(self.imdb.billboards_camera_positions[index]['camera_dx_xyzrpy']), 
                                          billboards_xyzrpy)

        

        return data_left, data_right, im_info, gt_boxes_left_padding, gt_boxes_right_padding,\
               gt_boxes_merge_padding, gt_dim_orien_padding, gt_kpts_padding, num_boxes, extrinsic_left, intrinsics_left, extrinsic_right, intrinsics_right
        ...
        data_right = data_right[0].permute(2, 0, 1).contiguous()
        im_info = im_info.view(3) 
        # print(data_left.max(), data_left.min())
        ...
        billboards_xyzrpy = [np.array(self.imdb.billboards_camera_positions[index]['billboard_xyzrpy']), 
                            np.array(self.imdb.billboards_camera_positions[index]['billboard2_xyzrpy'])]
        self.imdb.build_intrinsics(self.imdb.camera_params)
        extrinsic_left, intrinsics_left = self.imdb.build_extrinsics(np.array(self.imdb.billboards_camera_positions[index]['camera_xyzrpy']), 
                                          billboards_xyzrpy)
        
        self.imdb.build_intrinsics(self.imdb.camera_dx_params)
        extrinsic_right, intrinsics_right = self.imdb.build_extrinsics(np.array(self.imdb.billboards_camera_positions[index]['camera_dx_xyzrpy']), 
                                          billboards_xyzrpy)

        

        return data_left, data_right, im_info, gt_boxes, gt_boxes, gt_boxes, gt_boxes, gt_boxes, num_boxes, extrinsic_left, intrinsics_left, extrinsic_right, intrinsics_right
