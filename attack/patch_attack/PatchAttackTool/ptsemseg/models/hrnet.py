...
from ptsemseg.models.utils_for_hrnet.hrnet_config import MODEL_CONFIGS
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self,
        ...
        if cfg is None:
            # if cfg is none load the hrnetv42
    ...
    def forward(self, x):
        im_shape_x, im_shape_y = x.size(2), x.size(3)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.conv2(x)
        ...
        x = F.interpolate(x, size=(im_shape_x, im_shape_y), mode='bilinear', align_corners=ALIGN_CORNERS)
