...
#---------------------------------------------------------------------
def smooth_L1_loss_gamma(input, target, patch_mask, weight=None, size_average=True, gamma = 0.8, dist_threshold=0.5):
    ...
    input = input.view(-1) #transpose(1, 2).transpose(2, 3).contiguous().view(-1, c)
    ...
    input_only_misc = input.clone()
    input_only_no_misc = input.clone()
    # pred = torch.argmax(input, dim=1).to('cuda').detach()

    # target_only_misc[target==pred]  = 250
    # target_only_no_misc[target!=pred] = 250
    
    target_only_misc[torch.where(torch.abs(target_only_misc - input) > dist_threshold)] = 0
    target_only_no_misc[torch.where(torch.abs(target_only_misc - input) <= dist_threshold)] = 0

    input_only_misc[torch.where(target_only_misc == 0)] = 0
    input_only_no_misc[torch.where(target_only_no_misc == 0)] = 0
    
    target_only_misc[patch_mask==1]  = 0
    target_only_no_misc[patch_mask==1] = 0
    input_only_misc[patch_mask==1]  = 0
    input_only_no_misc[patch_mask==1] = 0
    ...
    #     loss_no_misc = F.smooth_l1_loss(
    #         input, target_without_patch, reduction='sum'
    #     )
    #     ret_gamma = 1.0
    ...
    loss_no_misc = F.smooth_l1_loss(
        input, target_only_no_misc, reduction='sum'
    )

    # loss for misclassified elements
    loss_misc = F.smooth_l1_loss(
        input, target_only_misc, reduction='sum'
    )
...
def cls_loss_od(input, target, weight=None, patch_mask = None, size_average=True, scale_weight=None, gamma = 0.9):
...
def bbox_loss_od(input, target, weight=None, patch_mask = None, size_average=True, scale_weight=None, gamma = 0.9):
    # 
    pass
