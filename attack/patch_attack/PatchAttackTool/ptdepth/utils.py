...
def get_model_state(state, model_name):
    ...
    if model_name in ['ddrnet23Slim', 'ddrnet23', 'hrnet']:
    ...
    elif model_name in ['fcn16s', 'fcn32s', 'AdaBinsBN']:
#         print(state.keys())
        return state

    elif model_name in ['GLPDepth']:
        if 'module' in next(iter(state.items()))[0]:
            state = OrderedDict((k[7:], v) for k, v in state.items())
        return state

    elif model_name in ['AdaBins']:
