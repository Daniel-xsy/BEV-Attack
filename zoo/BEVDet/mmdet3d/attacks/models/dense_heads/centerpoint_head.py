...
class CenterHead_Adv(BaseModule):
    ...
    def __init__(self,
        ...
        super(CenterHead_Adv, self).__init__(init_cfg=init_cfg)
    ...
    def get_bboxes(self, preds_dicts, img_metas, img=None, rescale=False):
        ...
        Custom for attack, return classification logits
        ...
        batch_logits = torch.cat([preds_dict[0]['heatmap'].sigmoid() for preds_dict in preds_dicts], dim=1)
            ...
            if nms_type == 'circle':
                ...
                rets.append(
                    self.get_task_detections(num_class_with_bg,
                                             ...
                                             batch_cls_labels, batch_cls_logits,
                                             img_metas, task_id))
        ...
        for i in range(num_samples):
            for k in rets[0][i].keys():
                if k == 'bboxes':
                    ...
                    logits = torch.cat([ret[i][k] for ret in rets], dim=0)
            ret_list.append([bboxes, scores, labels, logits])
        return ret_list

    def get_task_detections(self, num_class_with_bg, batch_cls_preds,
                            batch_reg_preds, batch_cls_labels, batch_cls_logits, img_metas, task_id):
        ...
        for i, (box_preds, cls_preds, cls_labels, logits) in enumerate(
