...
from .centerpoint import CenterPoint_Adv
from mmdet3d.models import builder


@DETECTORS.register_module()
class BEVDet_Adv(CenterPoint_Adv):
    def __init__(self, img_view_transformer, img_bev_encoder_backbone, img_bev_encoder_neck, **kwargs):
        super(BEVDet_Adv, self).__init__(**kwargs)
...
@DETECTORS.register_module()
class BEVDetSequentialES_Adv(BEVDetSequential_Adv):
    def __init__(self, before=False, interpolation_mode='bilinear',**kwargs):
        super(BEVDetSequentialES_Adv, self).__init__(**kwargs)
        self.before=before
...
@DETECTORS.register_module()
class BEVDepth_Adv(BEVDepth_Base, BEVDet_Adv):
    def extract_img_feat(self, img, img_metas):
        """Extract features of images."""
        x = self.image_encoder(img[0])
...
@DETECTORS.register_module()
class BEVDepth4D_Adv(BEVDepth_Base, BEVDetSequentialES_Adv):
    def extract_img_feat(self, img, img_metas):
        inputs = img
        """Extract features of images."""
        B, N, _, H, W = inputs[0].shape
