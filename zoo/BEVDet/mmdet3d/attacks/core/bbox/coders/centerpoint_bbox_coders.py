...
class CenterPointBBoxCoder_Adv(BaseBBoxCoder):
    """Bbox coder for CenterPoint.
    Custom modified for attack
    ...
    def _gather_logits(self, logits, topk_inds, batch, cat, cat_total):

        inds_ = topk_inds.unsqueeze(dim=1).repeat(1, cat_total, 1, 1)
        logits_ = logits.view(batch, cat_total, -1).unsqueeze(dim=2).repeat(1,1,cat,1)
        topk_logits = torch.gather(logits_, dim=-1, index=inds_)

        return topk_logits

    def _topk(self, scores, logits, K=80):
        ...
        topk_logits = self._gather_logits(logits, topk_inds, batch, cat, cat_total)

        topk_score, topk_ind = torch.topk(topk_scores.view(batch, -1), K)

        topk_ind_ = topk_ind.unsqueeze(dim=1).repeat(1, cat_total, 1)
        topk_logits_ = topk_logits.view(batch, cat_total, -1)
        topk_logit = torch.gather(topk_logits_, dim=-1, index=topk_ind_)
        ...
        return topk_score, topk_logit, topk_inds, topk_clses, topk_ys, topk_xs
    ...
    def decode(self,
        ...
        scores, logits, inds, clses, ys, xs = self._topk(heat, logits, K=self.max_num)
