...
class PatchAttack(BaseAttacker):
    ...
    def get_patch_mask(self, reference_points_cam, bev_mask, patch_mask, patch_size=torch.tensor((5,5))):
        ...
        if len(patch_size.size()) > 1:
            assert patch_size.size(-1) <= 2, f"Last dim of patch size should have size less than 2, now {patch_size.size(0)}"
